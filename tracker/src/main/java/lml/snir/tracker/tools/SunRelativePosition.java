 ...
 */
package lml.snir.tracker.tools;
...
import lml.snir.tracker.metier.MetierFactory;
 ...
 *
 * <UL>
 * <LI>L'azimuth du soleil (en degrés dans le sens des aiguilles d'une montre
 * depuis le nord);</LI>
 ...
 * </TR></TABLE>
 *
 * Les algorithmes utilisés dans cette classe sont des adaptations des
 * algorithmes en javascript écrit par le "National Oceanic and Atmospheric
 * Administration, Surface Radiation Research Branch". L'application original
 * est le
 *
 * <p>
 * <a href="http://www.srrb.noaa.gov/highlights/sunrise/azel.html">Solar
 * Position Calculator</a>.
 *
 * <p>
...
public class SunRelativePosition {
    ...
    /**
     * Valeur affectée lorsque un resultat n'est pas calculable du fait de la
     * nuit. Cette valeur concerne les valeurs de sorties {@link #elevation} et
     * {@link #azimuth}.
     */
    private static final double DARK = Double.NaN;

    /**
     * {@linkplain #getElevation Elevation angle} of astronomical twilight, in
     * degrees. Astronomical twilight is the time of morning or evening when the
     * sun is 18° below the horizon (solar elevation angle of -18°).
     */
    public static final double ASTRONOMICAL_TWILIGHT = -18;

    /**
     * {@linkplain #getElevation Elevation angle} of nautical twilight, in
     * degrees. Nautical twilight is the time of morning or evening when the sun
     * is 12° below the horizon (solar elevation angle of -12°).
     */
     ...
     * Common values are defined for the {@linkplain #ASTRONOMICAL_TWILIGHT astronomical twilight} (-18°), {@linkplain
     * #NAUTICAL_TWILIGHT nautical twilight} (-12°) and
     * {@linkplain #CIVIL_TWILIGHT civil twilight} (-6°). If no twilight are
     * defined, then this value is {@linkplain Double#NaN NaN}. The
     * {@linkplain #getElevation elevation} and {@linkplain #getAzimuth azimuth}
     * are set to {@linkplain Double#NaN NaN} when the sun elevation is below
     * the twilight value (i.e. during night). The default value is
     * {@link #CIVIL_TWILIGHT}.
     */
    private double twilight = CIVIL_TWILIGHT;

    /**
     * Heure à laquelle le soleil est au plus haut dans la journée en
     * millisecondes écoulées depuis le 1er janvier 1970.
     */
    private long noonTime;

    /**
     * Azimuth du soleil, en degrés dans le sens des aiguilles d'une montre
     * depuis le nord.
     */
    ...
    /**
     * Date and time when the current elevation and azimuth were computed. Value
     * is in milliseconds ellapsed since midnight UTC, January 1st, 1970.
     */
    private long time = System.currentTimeMillis();

    /**
     * {@code true} is the elevation and azimuth are computed, or {@code false}
     * if they need to be computed. This flag is set to {@code false} when the
     * date and/or the coordinate change.
     */
     ...
     * {@code (a-b)/a} where
     * <var>a</var> is the semi-major axis length and <var>b</var> is the
     * semi-minor axis length. Value is 0 for a circular orbit.
     *
     * @param t number of Julian centuries since J2000.
...
//    * @return 
//    */
//    private static double equationOfTime(int n){
//        double b = ((2*Math.PI*(n-81))/365);
//        double eqTime = 7.678*Math.sin(b+1.374)-9.87*Math.sin(2*b);
//        
//        return Math.toDegrees(eqTime);
//    }
    ...
    /**
     * Set the geographic coordinate where to compute the
     * {@linkplain #getElevation elevation} and
     * {@linkplain #getAzimuth azimuth}.
     *
     * @param longitude The longitude in degrees. Positive values are East;
     * negative values are West.
     * @param latitude The latitude in degrees. Positive values are North,
     ...
     * {@linkplain #getAzimuth azimuth}.
     *
     * @param point The geographic coordinates in degrees of longitude and
     * latitude.
     */
     ...
     * #getAzimuth azimuth} computation. This is the coordinate specified during
     * the last call to a
    ...
    /**
     * Set the date and time when to compute the
     * {@linkplain #getElevation elevation} and
     * {@linkplain #getAzimuth azimuth}.
     *
     ...
     * azimuth} computation. This is the date specified during the last call to
     * {@link #setDate}.
     */
    public Date getDate() {
    ...
    /**
     * Set the sun's {@linkplain #getElevation elevation angle} at twilight, in
     * degrees. Common values are defined for the {@linkplain #ASTRONOMICAL_TWILIGHT astronomical twilight} (-18°),
     ...
     * azimuth} are set to {@linkplain Double#NaN NaN} when the sun elevation is
     * below the twilight value (i.e. during night). The default value is
     * {@link #CIVIL_TWILIGHT}.
     *
     * @param twilight The new sun elevation at twilight, or {@link Double#NaN}
     * if no twilight value should be taken in account.
    ...
    /**
     * Returns the sun's {@linkplain #getElevation elevation angle} at twilight,
     * in degrees. This is the value set during the last call to
     * {@link #setTwilight}.
     */
    ...
    /**
     * Affiche la position du soleil à la date et coordonnées spécifiée. Cette
     ...
     *
     * où <var>date</var> est un argument optionel spécifiant la date et
     * l'heure. Si cet argument est omis, la date et heure actuelles seront
     * utilisées.
     */
